...
function getName(x){return(x.customer&&x.customer.name)||x.customer_name||"؟";}
function getTime(x){return x.start_time?new Date(x.start_time).toLocaleTimeString("ar-IQ",{hour:"2-digit",minute:"2-digit"}):"؟";}
function getProvider(x){return(x.provider&&x.provider.full_name)||x.provider_name||"غير محدد";}

function calc(data){
var all=Array.isArray(data)?data:(data.results||[]);

// فلتر اليوم فقط
var today=new Date();
var todayStr=today.getFullYear()+"-"+String(today.getMonth()+1).padStart(2,"0")+"-"+String(today.getDate()).padStart(2,"0");
var a=all.filter(function(x){
  var d=x.start_time||x.date||"";
  return d.startsWith(todayStr);
});

if(!a.length){E("لا توجد حجوزات اليوم\n("+todayStr+")");return;}
...
var bp={};
a.forEach(function(x){
  if(!x.start_time||[7,8,13,29].indexOf(x.status_id)>-1)return;
  var p=getProvider(x);
...
Object.keys(bp).forEach(function(pv){
  ...
  for(var i=0;i<l.length;i++)for(var j=i+1;j<l.length;j++){
    var s1=new Date(l[i].start_time).getTime();
    var e1=new Date(l[i].end_time).getTime();
    var s2=new Date(l[j].start_time).getTime();
    var e2=new Date(l[j].end_time).getTime();
    if(s1<e2&&s2<e1)st.cf.push({pv:pv,p1:getName(l[i]),p2:getName(l[j]),t1:getTime(l[i])});
  }
});
...
var h='<div style="color:#8892a4;font-size:11px;font-weight:700;margin-bottom:8px">📊 إحصائيات اليوم — '+todayStr+'</div>';
...
h+='</div>';
h+='<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin-bottom:9px">';
h+=C(st.ar,"#22c55e","وصل")+C(st.ea,"#10b981","مبكر")+C(st.an,"#8b5cf6","تخدير");
h+=C(st.co,"#06b6d4","خرج")+C(st.rd,"#ec4899","تم التذكير")+C(st.pe,"#f59e0b","Pending");
h+='</div>';
h+='<div style="text-align:center;color:#8892a4;font-size:11px;margin-bottom:9px;padding-bottom:9px;border-bottom:1px solid #2d3548">الإجمالي: '+st.total+' حجز</div>';
h+='<div style="color:#8892a4;font-size:11px;font-weight:700;margin-bottom:8px">⚠️ تنبيهات</div>';

var ha=false;
if(st.cf.length){ha=true;h+=A("red","🔴 تضارب ("+st.cf.length+")",st.cf.slice(0,3).map(function(c){return"• "+c.pv+": "+c.p1+" ↔ "+c.p2;}).join("<br>"));}
if(st.al>0){ha=true;h+=A("orange","🟠 "+st.al+" متأخر",st.lp.slice(0,4).map(function(p){return"• "+getName(p)+" — "+getTime(p);}).join("<br>"));}

(function(){
var o=document.getElementById("fb");if(o){o.remove();return;}
...
function calc(data){
var a=data.results||data;
if(!Array.isArray(a)||!a.length){E("لا توجد بيانات");return;}
var st={total:a.length,ar:0,al:0,ie:0,an:0,rd:0,ca:0,ns:0,co:0,nr:0,pe:0,ea:0,lp:[],cf:[]};
a.forEach(function(x){var id=x.appt_status;if(id==2)st.ar++;else if(id==3){st.al++;st.lp.push(x);}else if(id==4)st.ie++;else if(id==5)st.an++;else if(id==6)st.rd++;else if(id==7||id==13||id==29)st.ca++;else if(id==8)st.ns++;else if(id==9)st.co++;else if(id==10)st.nr++;else if(id==11)st.pe++;else if(id==12)st.ea++;});
var bp={};
a.forEach(function(x){if(!x.appt_time||[7,8,13,29].indexOf(x.appt_status)>-1)return;var p=x.provider_name||"غير محدد";if(!bp[p])bp[p]=[];bp[p].push(x);});
Object.keys(bp).forEach(function(pv){var l=bp[pv];for(var i=0;i<l.length;i++)for(var j=i+1;j<l.length;j++){var ra=R(l[i].appt_time),rb=R(l[j].appt_time);if(ra&&rb&&ra.s<rb.e&&rb.s<ra.e)st.cf.push({pv:pv,p1:l[i].customer_name||"؟",p2:l[j].customer_name||"؟"});}});
...
h+=C(ps,"#22c55e","حضروا")+C(st.al,"#f97316","متأخر")+C(st.ns,"#ef4444","No Show")+C(st.ca,"#6b7280","ملغي")+C(st.ie,"#3b82f6","بالفحص")+C(wt,"#a855f7","انتظار");
...
if(st.al>0){ha=true;h+=A("orange","🟠 "+st.al+" متأخر",st.lp.slice(0,3).map(function(p){return"• "+(p.customer_name||"؟")+" — "+(p.appt_time||"");}).join("<br>"));}
...
fetch(url,{headers:{"Authorization":"Bearer "+tk}}).then(function(r){return r.json();}).then(function(d){calc(d);}).catch(function(e){E("خطأ: "+e.message);});

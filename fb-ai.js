...
function getProvider(x){return(x.provider&&x.provider.full_name)||x.provider_name||x.provider_id||"غير محدد";}
...
var ha=false;
if(st.cf.length){ha=true;h+=A("red","🔴 تضارب ("+st.cf.length+")",st.cf.slice(0,3).map(function(c){return"• "+c.pv+": "+c.p1+" ↔ "+c.p2+" ("+c.t1+")";}).join("<br>"));}
...
if(st.nr>0){ha=true;h+=A("orange","🟠 "+st.nr+" Not in Reception","");}
